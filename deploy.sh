...
then
    ## develop.dxperts.org subdomain (independent repo)
    echo "Pushing new develop subdomain repo"
    git clone https://github.com:${GITHUB_TOKEN}@github.com/${DEVELOP_REPO} $TRAVIS_BUILD_DIR/develop.dxperts.org
    cd $TRAVIS_BUILD_DIR/develop.dxperts.org
    rm -rf ./*
    git checkout ./CNAME
...
then
    ## staging.dxperts.org subdomain (independent repo)
    echo "Pushing new staging subdomain repo"
    git clone https://github.com:${GITHUB_TOKEN}@github.com/${STAGING_REPO} $TRAVIS_BUILD_DIR/staging.dxperts.org
    cd $TRAVIS_BUILD_DIR/staging.dxperts.org
    rm -rf ./*
    git checkout ./CNAME
    cp -Rv $TRAVIS_BUILD_DIR/build/hash-history/* .
    git add -A
...
then
    ## wallet.dxperts.org subdomain (independent repo)
    echo "Pushing new wallet subdomain repo"
    git clone https://github.com:${GITHUB_TOKEN}@github.com/${WALLET_REPO} $TRAVIS_BUILD_DIR/wallet.dxperts.org
    cd $TRAVIS_BUILD_DIR/wallet.dxperts.org
