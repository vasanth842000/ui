...
// Filter DXP 0.9.2+ import keys export file so that it will include only private keys
// that may be found in the DXP 2.0 genesis block.
// Dependencies:
// ./bloom_dxperts.dat (1,048,576 bytes) sha1 4d80faa41a5e868899febdc9dab48d1f2d567487992810cf0532f3c0ee2b266c
// nodejs, npm, and: npm install

/*
 * ./bloom.dat is from dxperts/graphene/programs/genesis_util/create_bloom_filter.py
 * The bloom filter was created with a genesis containing DXP prefixed keys.  Create
...
var graphenejs = require("dxpertsjs");
var h = graphenejs.hash;
var key_utils = graphenejs.key;
const chainPrefix = "DXP";

fs.readFile("bloom_dxperts.dat", function(err, data) {
    if (err) throw err;
    console.error(
        "bloom_dxperts.dat (" + data.length + " bytes)",
        "sha1",
