...
import {ChainStore, ChainTypes} from "dxpertsjs";
...
const MarketUtils = {
    ...
    limitByPrecision(value, asset, floor = true) {
        ...
        let precision = utils.get_token_precision(assetPrecision);
        value = floor
    ...
    getFeedPrice(some_raw_price, invert = false) {
        ...
        let price = utils.get_token_price(
    ...
    parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/DXP, amount should be in DXP, value should be in USD, debt is in USD
                // buy is in USD, sell is in DXP
                // quote is USD, base is DXP
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in DXP/USD, amount should be in USD, value should be in DXP, debt is in USD
                // buy is in USD, sell is in DXP
                // quote is USD, base is DXP
    ...
    isMarketAsset(quote, base) {
        ...
        if (
            quote.get("smarttoken") &&
            base.get("id") ===
                quote.getIn(["smarttoken", "options", "short_backing_asset"])
        ) {
            isMarketAsset = true;
            ...
            base.get("smarttoken") &&
            quote.get("id") ===
                base.getIn(["smarttoken", "options", "short_backing_asset"])
        ) {
            inverted = true;
            isMarketAsset = true;
