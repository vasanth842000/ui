...
var Utils = {
    ...
    get_token_precision: precision => {
        precision = precision.toJS ? precision.get("precision") : precision;
        return Math.pow(10, precision);
    },

    get_token_amount: function(amount, asset) {
        if (amount === 0) return amount;
        if (!amount) return null;
        return (
            amount /
            this.get_token_precision(
    ...
    get_token_price: function(
        ...
        var price =
            this.get_token_amount(quoteAmount, quoteAsset) /
            this.get_token_amount(baseAmount, baseAsset);
    ...
    format_asset: function(amount, asset, noSymbol, trailing_zeros = true) {
        ...
        let precision = this.get_token_precision(digits);
    ...
    convertPrice: function(fromRate, toRate, fromID, toID) {
        ...
        if (fromRate.toJS && this.is_object_type(fromRate.get("id"), "asset")) {
            ...
            fromRate = fromRate.get("smarttoken")
    ...
    convertValue: function(priceObject, amount, fromAsset, toAsset) {
        ...
        let quotePrecision = this.get_token_precision(
            fromAsset.get("precision")
        );
        let basePrecision = this.get_token_precision(toAsset.get("precision"));

        let assetPrice = this.get_token_price(
    ...
    replaceName(asset) {
        if (!asset) return {name: "", prefix: null, isSmartToken: false};
        let name = asset.get("symbol");
        const isSmartToken =
            asset.get("smarttoken") &&
            !asset.getIn(["smarttoken", "is_prediction_market"]) &&
        ...
        let namespace = isSmartToken ? "bit" : toReplace[i];
        let prefix = null;
