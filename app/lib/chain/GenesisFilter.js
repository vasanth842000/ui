import {hash, key} from "dxpertsjs";

var dxp_genesiskeys_bloom_url = undefined;
try {
    ...
    dxp_genesiskeys_bloom_url = url;
} catch (e) {
    // webpack deployment exception (not run time)
    console.log(
        "WARN: Will be unable to filter DXP 1.0 wallet imports, did not find assets/bts_genesiskeys_bloom.dat",
        e
    );
}

/**
    This should only be applied to a DXP 1.0 export file taken on the
    discontinued chain. Any public key string or address (all 5 formats) carried
    over to the DXP 2.0 genesis block will be in this filter.
...
export default class GenesisFilter {
    ...
    isAvailable() {
        return dxp_genesiskeys_bloom_url !== undefined;
    }
    ...
    init(done) {
        ...
        xhr.open("GET", dxp_genesiskeys_bloom_url);
        xhr.send();
    }
    ...
    filter(account_keys, status) {
        if (!this.isAvailable()) {
            console.log("WARN: Missing bloom filter for DXP 0.9.x wallets");
                ...
                for (var a = 0; a < account_keys.length; a++) {
                    ...
                    for (
                        ...
                        if (/^GPH/.test(currentKey))
                            currentKey = "DXP" + currentKey.substring(3);
                        if (this.inGenesis(currentKey)) continue;
                        var addresses = key.addresses(currentKey, "DXP");
