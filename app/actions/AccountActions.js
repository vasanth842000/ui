...
class AccountActions {
    ...
    createDxpcore({url, account}) {
        const account_id = account.get("id");
        var tr = WalletApi.new_transaction();

        tr.add_type_operation("dxpcore_member_create", {
            fee: {
                amount: 0,
                asset_id: "1.3.0"
            },
            dxpcore_member_account: account_id,
            url: url
        });
        return dispatch => {
            return WalletDb.process_transaction(tr, null, true)
                ...
                .catch(error => {
                    console.log("----- Add Dxpcore member error ----->", error);
                    dispatch(false);
                });
        };
    }

    createBlockproducer({url, account, signingKey}) {
        const account_id = account.get("id");
        var tr = WalletApi.new_transaction();

        tr.add_type_operation("blockproducer_create", {
            fee: {
                amount: 0,
                asset_id: "1.3.0"
            },
            blockproducer_account: account_id,
            url: url,
