...
import {Modal, Button} from "dxperts-ui-style-guide";
 ...
 *
 *  Expected Properties:
 *     quoteAssetObj:  asset id, must be a smarttoken
...
class BorrowModalContent extends React.Component {
    ...
    _initialState(props) {
        ...
        if (currentPosition.collateral) {
            let debt = utils.get_token_amount(
                currentPosition.debt,
                props.quoteAssetObj
            );
            let collateral = utils.get_token_amount(
    ...
    _maximizeDebt() {
        ...
        if (currentPosition.collateral) {
            initialCollateral = utils.get_token_amount(
                currentPosition.collateral,
                this.props.backingAssetObj
            );
        }
        ...
        let maximumCollateral =
            this.props.collateralBalanceObj.get("balance") /
                utils.get_token_precision(this.props.backingAssetObj) +
            initialCollateral -
            10;
        const debtAmount =
    ...
    _payDebt() {
        ...
        const debtAmount = utils.get_token_amount(
            Math.max(
    ...
    _onSubmit(e) {
        ...
        let backingPrecision = utils.get_token_precision(
            this.props.backingAssetObj.get("precision")
        );
    ...
    _isPredictionMarket(props) {
        return props.quoteAssetObj.getIn([
            "smarttoken",
    ...
    render() {
        ...
        let debtPrecision = utils.get_token_precision(
            this.props.quoteAssetObj.get("precision")
        );
...
export default class ModalWrapper extends React.Component {
    ...
    render() {
        ...
        let coreBalance, smartTokenBalance;
        ...
        if (accountObjBalance) {
            for (var id in accountObjBalance) {
                ...
                if (id === quoteAssetObj) {
                    smartTokenBalance = accountObjBalance[id];
                }
            }
        }
        ...
        return this.props.visible ? (
            <BorrowModalContent
                ...
                debtBalanceObj={smartTokenBalance}
