...
import {Modal, Button, Select, Input} from "dxperts-ui-style-guide";
...
class WithdrawModalNew extends React.Component {
    ...
    _getAssetPairVariables(props = this.props, state = this.state) {
        ...
        let isDXP = false;
        if (coreAsset) {
            if (selectedAsset == coreAsset.get("symbol")) isDXP = true;
        } else if (selectedAsset == "DXP") {
            isDXP = true;
        }
    ...
    _getAvailableAssets(state = this.state) {
        let dxpAccount = this.props.account;
        let fee_asset_types = [];
        if (!(dxpAccount && dxpAccount.get("balances"))) {
            return {fee_asset_types};
        }
        let account_balances = dxpAccount.get("balances").toJS();
    ...
    onDXPAccountNameChanged(dxpAccountName) {
        if (!dxpAccountName) this.setState({dxpAccount: null});
        this.setState({dxpAccountName, dxpAccountError: null});
    }

    onDXPAccountChanged(dxpAccount) {
        this.setState({dxpAccount, dxpAccountError: null});
    }
            ...
            minWithdraw = !!backingAsset.precision
                ? utils.format_number(
                      backingAsset.minAmount /
                          utils.get_token_precision(backingAsset.precision),
            ...
            >
                <div className="grid-block vertical no-overflow">
                    <div className="modal__body" style={{paddingTop: 0}}>
                        ...
                        {!isDXP && selectedAsset && !selectedGateway ? (
                        ...
                        {/*ESTIMATED VALUE*/}
                        {/*
                (assetAndGateway || quantity) && !isDXP ?
                <div>
            ...
            */}
                        ...
                        {isDXP ? (
                            <div style={{marginBottom: "1em"}}>
                                <AccountSelector
                                    label="transfer.to"
                                    accountName={state.dxpAccountName}
                                    onChange={this.onDXPAccountNameChanged.bind(
                                        this
                                    )}
                                    onAccountChanged={this.onDXPAccountChanged.bind(
                                        this
                                    )}
                                    account={state.dxpAccountName}
                                    size={60}
                                    error={state.dxpAccountError}
                                />
                            </div>
                        ) : null}
