...
class AssetName extends React.Component {
    ...
    render() {
        ...
        const isSmartToken = asset.has("smarttoken");
        const isPredMarket =
            isSmartToken && asset.getIn(["smarttoken", "is_prediction_market"]);

        let {name: replacedName, prefix} = utils.replaceName(asset);
        const hasBitPrefix = prefix === "bit";

        let includeSmartTokenDescription =
            isSmartToken && !isPredMarket && hasBitPrefix;

        if ((replace && replacedName !== name) || isSmartToken) {
            ...
            try {
                optional =
                    realPrefix || includeSmartTokenDescription
                        ? counterpart.translate(
                              ...
                              {
                                  asset: name,
                                  backed: includeSmartTokenDescription
                                      ? desc.main
                                      : replacedName
                              }
                          )
            ...
            if (isSmartToken && name === "CNY") {
                optional =
                    optional +
                    " " +
                    counterpart.translate("gateway.assets.dcny");
            }
            ...
            if (!!noTip) {
                ...
                let desc = null;
                if (replacedName == "DXP") {
                    desc = {main: counterpart.translate("assets.DXP")};
                } else {
