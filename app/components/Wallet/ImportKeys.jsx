...
import {PrivateKey, Aes, PublicKey, FetchChain, hash} from "dxpertsjs";
import AccountApi from "api/accountApi";
import {ChainConfig} from "dxpertsjs-ws";
...
import {Button, Input} from "dxperts-ui-style-guide";
...
class ImportKeys extends Component {
    ...
    /** DXP 1.0 client wallet_export_keys format. */
    _parseImportKeyUpload(json_contents, file_name, update_state) {
        ...
        // DXP 1.0 wallets may have a lot of generated but unused keys or spent TITAN addresses making
        ...
        this.setState(
            ...
            () =>
                ...
                genesis_filter.init(() => {
                    ...
                    // FF < version 41 does not support benefactor threads internals (like blob urls)
                    // let GenesisFilterBenefactor = require("benefactor-loader!benefactors/GenesisFilterBenefactor")
                    // let benefactor = new GenesisFilterBenefactor
                    // benefactor.postMessage({
                    //     account_keys: unfiltered_account_keys,
                    //     bloom_filter: genesis_filter.bloom_filter
                    // })
                    // benefactor.onmessage = event => { try {
                    //     let { status, account_keys } = event.data
                    //     // ...
                    // } catch( e ) { console.error('GenesisFilterBenefactor', e) }}
                    ...
                    genesis_filter.filter(account_keys, status => {
                        ...
                        if (status.success) {
                            // let { account_keys } = event.data // if using benefactor thread
                            update_state({
                                password_checksum,
                                account_keys,
                                genesis_filter_finished: true,
    ...
    /**
    DXP 1.0 hosted wallet backup (wallet.dxperts.org) is supported.

    DXP 1.0 native wallets should use wallet_export_keys instead of a wallet backup.
    ...
    */
    _parseWalletJson(json_contents) {
        ...
        let savePubkeyAccount = function(pubkey, account_name) {
            ...
            address = "DXP" + address.substring(3);
        ...
        try {
            ...
            if (!encrypted_brainkey)
                throw "Please use a DXP 1.0 wallet_export_keys file instead";
    ...
    saveImport() {
        ...
        this.reset();
        WalletDb.importKeysBenefactor(private_key_objs)
            .then(result => {
                        ...
                        <div>
                            <div>
                                {privateKey ? (
                                    ...
                                    >
                                        <label>
                                            <Translate content="wallet.dxp_09_export" />
