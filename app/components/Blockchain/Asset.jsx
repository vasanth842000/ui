...
import {Tooltip, Icon, Table, Tabs, Collapse} from "dxperts-ui-style-guide";
...
class Asset extends React.Component {
    ...
    _getMarginCollateral() {
        if (this.props.asset.has("smarttoken")) {
            const assets = {
            ...
            const isPredictionMarket = this.props.asset.getIn(
                ["smarttoken", "is_prediction_market"],
                false
            );
    ...
    _assetType(asset) {
        return "smarttoken" in asset
            ? asset.smarttoken.is_prediction_market
                ? "Prediction"
                : "Smart"
            : "Simple";
    }
    ...
    renderAboutBox(asset, originalAsset) {
        ...
        if (asset.smarttoken) {
            preferredMarket = ChainStore.getAsset(
                asset.smarttoken.options.short_backing_asset
            );
    ...
    renderSummary(asset) {
        ...
        let flagBooleans = assetUtils.getFlagBooleans(
            asset.options.flags,
            this.props.asset.has("smarttoken_data_id")
        );

        let bitNames = Object.keys(flagBooleans);

        let isPrediction =
            "smarttoken" in asset && asset.smarttoken.is_prediction_market;
    ...
    renderPriceFeed(asset) {
        var smartToken = asset.smarttoken;
        if (!("current_feed" in smartToken)) return <div header={title} />;
        var currentFeed = smartToken.current_feed;
                ...
                >
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                {smartToken.options.feed_lifetime_sec / 60 / 60}
                            </td>
                        </tr>
                        <tr>
                            <td>
                            ...
                            <td>{smartToken.options.minimum_feeds}</td>
                        </tr>
                        <tr>
                            <td>
    ...
    renderSettlement(asset) {
        ...
        var currentFeed = smartToken.current_feed;
        var isGlobalSettle =
            asset.smarttoken.settlement_fund > 0 ? true : false;
        ...
        if (isGlobalSettle) {
             ...
             */
            var settlementFund = smartToken.settlement_fund;
            ...
            let current_collateral_value = smartToken.settlement_fund;
            ...
            revive_price_with_bids = (
                <FormattedPrice
                    base_amount={
                        smartToken.settlement_fund / 1 + bids.collateral
                    } // /1 is implicit type conversion
                    base_asset={
                        assets[smartToken.options.short_backing_asset].id
                    }
            ...
            current_collateral_value =
                ...
                Math.pow(
                    10,
                    assets[smartToken.options.short_backing_asset].precision
                );
            ...
            var currentSettled = smartToken.force_settled_volume;
            var settlementOffset =
                smartToken.options.force_settlement_offset_percent;
            var settlementDelay = smartToken.options.force_settlement_delay_sec;
            var maxSettlementVolume =
                smartToken.options.maximum_force_settlement_volume;
        ...
        return (
            ...
            >
                ...
                <BidCollateralOperation
                    asset={asset.symbol}
                    core={asset.smarttoken.options.short_backing_asset}
    ...
    renderPermissions(asset) {
        ...
        // options.whitelist_markets = ["USD", "EUR"];
    ...
    _renderFeedTable(asset) {
        ...
        if (
            !("feeds" in smartToken) ||
            smartToken.feeds.length == 0 ||
            smartToken.is_prediction_market ||
            !smartToken.feeds.length
        ) {
            return null;
        }

        var feeds = smartToken.feeds;
        ...
        let oldestValidDate = new Date(
            now - asset.smarttoken.options.feed_lifetime_sec * 1000
        );
        feeds = feeds
            .filter(a => {
    ...
    renderFeedTables(asset) {
        ...
        let isGlobalSettlement = smartToken.settlement_fund > 0 ? true : false;
    ...
    render() {
        ...
        var priceFeed =
            "smarttoken" in asset ? this.renderPriceFeed(asset) : null;
        var priceFeedData =
            "smarttoken" in asset ? this.renderFeedTables(asset) : null;
...
class AssetContainer extends React.Component {
    render() {
        ...
        let backingAsset = this.props.asset.has("smarttoken")
