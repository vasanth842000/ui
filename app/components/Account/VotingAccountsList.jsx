...
function getBlockproducerOrDxpcore(type, acct) {
    let url = "",
        votes = 0,
        account;
    if (type === "blockproducer") {
        account = ChainStore.getBlockproducerById(acct.get("id"));
    } else if (type === "dxpcore") {
        account = ChainStore.getDxpcoreMemberById(acct.get("id"));
    }
...
const accountItemRow = props => {
    ...
    let {url, votes} = getBlockproducerOrDxpcore(type, account);
...
class VotingAccountsList extends React.Component {
    ...
    render() {
        ...
        let item_rows = this.props.items
            ...
            .sort((a, b) => {
                let {votes: a_votes} = getBlockproducerOrDxpcore(
                    this.props.type,
                    a
                );
                let {votes: b_votes} = getBlockproducerOrDxpcore(
                    this.props.type,
                    b
                );
                if (a_votes !== b_votes) {
            ...
            .map((i, idx) => {
                ...
                let isActive = this.props.active.includes(
                    getBlockproducerOrDxpcore(this.props.type, i).id
                );
