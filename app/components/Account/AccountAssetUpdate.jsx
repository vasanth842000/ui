...
import {SmartTokenOptions} from "./AccountAssetCreate";
...
class AccountAssetUpdate extends React.Component {
    ...
    resetState(props) {
        ...
        let isSmartToken = asset.smarttoken_data_id !== undefined;
        let precision = utils.get_token_precision(asset.precision);
        ...
        return {
            ...
            claimFeesAmount: 0,
            smarttoken_opts: isSmartToken ? asset.smarttoken.options : null,
            original_smarttoken_opts: isSmartToken
                ? props.asset.getIn(["smarttoken", "options"]).toJS()
                : null,
            marketInput: "",
            ...
            feedProducers: isSmartToken
                ? props.asset.getIn(["smarttoken", "feeds"], []).map(a => {
                      return a.first();
                  })
                : null,
            originalFeedProducers: isSmartToken
                ? props.asset.getIn(["smarttoken", "feeds"], []).map(a => {
                      return a.first();
                  })
                : null
    ...
    tabsChanged() {
        ...
        /* SmartToken options */
        if (
            JSON.stringify(s.smarttoken_opts) !==
            JSON.stringify(p.original_smarttoken_opts)
        )
    ...
    pageChanged() {
        ...
        return (
            this.assetChanged() ||
            (isSmartToken &&
                (JSON.stringify(smarttoken_opts) !==
                    JSON.stringify(original_smarttoken_opts) ||
    ...
    _updateAsset(e) {
        ...
        let feedProducersJS = isSmartToken ? feedProducers.toJS() : null;
        let originalFeedProducersJS = isSmartToken
            ? originalFeedProducers.toJS()
            : null;
    ...
    onChangeSmartTokenOpts(value, e) {
        let {smarttoken_opts} = this.state;
    ...
    _onUpdateInput(value, e) {
        let {update} = this.state;
        let updateState = true;
        let precision = utils.get_token_precision(
    ...
    _validateEditFields(new_state) {
        ...
        if (
            flagBooleans.dxpcore_fed_asset &&
            flagBooleans.blockproducer_fed_asset
        ) {
    ...
    _onPermissionChange(key) {
        const {isSmartToken, permissionBooleans} = this.state;
    ...
    render() {
        ...
        let basePrecision = utils.get_token_precision(
        ...
        let originalPermissions = assetUtils.getFlagBooleans(
            ...
            asset.get("smarttoken") !== undefined
        );
        // Loop over flags
        let flags = [];
                ...
                <div className="content-block small-12">
                    <div className="tabs-container generic-bordered-box">
                        <div className="tabs-header">
                            <h3>
                                <Translate content="header.update_token" />:{" "}
                                {symbol}
                            </h3>
                        </div>
                        <Tabs
                        ...
                        >
                            ...
                            {isSmartToken ? (
                                <Tab
                                    title="account.user_issued_assets.smarttoken_opts"
            ...
            >
                ...
                <div>
                    <ul>
                        ...
                        {tabsChanged["3"] ? (
                            <li>
                                <Translate content="account.user_issued_assets.smarttoken_opts" />
                            </li>
                        ) : null}
