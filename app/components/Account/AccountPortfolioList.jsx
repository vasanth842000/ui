...
import {Tooltip, Icon as AntIcon} from "dxperts-ui-style-guide";
...
class AccountPortfolioList extends React.Component {
    ...
    getHeader(atLeastOneHas) {
        ...
        let headerItems = [
            ...
            {
                className: "column-hide-medium",
                title: atLeastOneHas.depositOnlyDXP ? (
                    <React.Fragment>
                        <Tooltip
    ...
    _renderBalances(balanceList, optionalAssets, visible) {
        ...
        const renderBorrow = (asset, account) => {
            let isSmartToken = asset && asset.has("smarttoken_data_id");
            let isGlobalSettled =
                isSmartToken &&
                asset.getIn(["smarttoken", "settlement_fund"]) > 0
                    ? true
                    : false;
            ...
            return {
                isSmartToken,
                borrowLink:
                    !isSmartToken || isGlobalSettled ? null : (
                        <a
                            onClick={() => {
                                ReactTooltip.hide();
        ...
        (balanceList || []).forEach(balance => {
            ...
            let {isSmartToken, borrowLink} = renderBorrow(
                asset,
                this.props.account
            );
            ...
            let options =
                asset && asset.getIn(["smarttoken", "options"])
                    ? asset.getIn(["smarttoken", "options"]).toJS()
                    : null;
            let backingAsset =
                options && options.short_backing_asset
                    ? ChainStore.getAsset(options.short_backing_asset)
                    : null;
            let {isSmartToken: isAssetSmartToken} = utils.replaceName(asset);
            let {isSmartToken: isBackingSmartToken} = utils.replaceName(
                backingAsset
            );
            ...
            if (isSmartToken) {
                const globally_settled =
                    asset.get("smarttoken").get("settlement_fund") > 0;
            ...
            balances.push({
                ...
                borrow:
                    isSmartToken && borrowLink ? (
                        <Tooltip
                            title={counterpart.translate("tooltip.borrow", {
                                asset: isAssetSmartToken
                                    ? "bit" + symbol
                                    : symbol
                            })}
                        >
                        ...
                        <Tooltip
                            ...
                            title={counterpart.translate(settlePriceTitle, {
                                asset: isAssetSmartToken
                                    ? "bit" + symbol
                                    : symbol,
                                backingAsset: isBackingSmartToken
    ...
    render() {
        ...
        balanceRows.forEach(_item => {
            ...
            if (!!_item.deposit && _item.deposit !== "-") {
                if (_item.key == "DXP" && GatewayStore.anyAllowed()) {
                    atLeastOneHas.depositOnlyDXP =
                        _item.key == "DXP" && !atLeastOneHas.deposit;
                ...
                >
                    ...
                    {(this.state.isBurnModalVisible ||
                        ...
                        <ReserveAssetModal
                            ...
                            account={this.props.account}
                            onClose={() => {
                                ZfApi.publish("reserve_token", "close");
                            }}
                        />
                    )}
                </CustomTable>
