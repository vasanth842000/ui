...
export default class ExchangeHeader extends React.Component {
    ...
    render() {
        ...
        const lookForSmartToken =
            quoteId === "1.3.0" ? baseId : baseId === "1.3.0" ? quoteId : null;
        const possibleSmartToken = lookForSmartToken
            ? ChainStore.getAsset(lookForSmartToken)
            : null;
        const isSmartToken = possibleSmartToken
            ? !!possibleSmartToken.get("smarttoken")
            : false;
        let collOrderObject = "";
        ...
        if (isSmartToken) {
            if (account.toJS && account.has("call_orders")) {
                ...
                for (let i = 0; i < call_orders.length; i++) {
                    ...
                    if (debtAsset === lookForSmartToken) {
            ...
            // globally settled
            if (
                possibleSmartToken.get("smarttoken").get("settlement_fund") > 0
            ) {
                ...
                settlePrice = possibleSmartToken
                    .get("smarttoken")
