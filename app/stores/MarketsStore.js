...
class MarketsStore {
    ...
    onSubscribeMarket(result) {
        ...
        /* Set the feed price (null if not a smarttoken market) */
    ...
    _getFeed() {
        ...
        try {
            ...
            this.is_prediction_market = this[
                this.invertedCalls ? "baseAsset" : "quoteAsset"
            ].getIn(["smarttoken", "is_prediction_market"], false);
            this.smarttoken_options = this[
                this.invertedCalls ? "baseAsset" : "quoteAsset"
            ]
                .getIn(["smarttoken", "options"])
                .toJS();
            ...
            if (
                ...
                const backingAsset = this.smarttoken_options
                    .short_backing_asset;
    ...
    _priceChart() {
        ...
        for (let i = 0; i < this.priceHistory.length; i++) {
            ...
            if (this.quoteAsset.get("id") === current.key.quote) {
                high = utils.get_token_price(
                    current.high_base,
                    this.baseAsset,
                    current.high_quote,
                    this.quoteAsset
                );
                low = utils.get_token_price(
                    current.low_base,
                    this.baseAsset,
                    current.low_quote,
                    this.quoteAsset
                );
                open = utils.get_token_price(
                    current.open_base,
                    this.baseAsset,
                    current.open_quote,
                    this.quoteAsset
                );
                close = utils.get_token_price(
                    current.close_base,
                    this.baseAsset,
                    current.close_quote,
                    this.quoteAsset
                );
                volume = utils.get_token_amount(
