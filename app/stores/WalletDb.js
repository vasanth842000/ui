...
import {ChainStore, PrivateKey, key, Aes} from "dxpertsjs";
...
let dictJson, AesBenefactor;
if (__ELECTRON__) {
    AesBenefactor = require("worker-loader?inline!benefactors/AesBenefactor");
...
class WalletDb extends BaseStore {
    ...
    */
    ...
    importKeysBenefactor(private_key_objs) {
        return new Promise((resolve, reject) => {
            let pubkeys = [];
            ...
            if (!__ELECTRON__) {
                AesBenefactor = require("worker-loader!benefactors/AesBenefactor");
            }
            let benefactor = new AesBenefactor();
            ...
            benefactor.onmessage = event => {
                try {
                    ...
                    console.error("AesBenefactor.encrypt", e);
                }
            };
        });
    }
