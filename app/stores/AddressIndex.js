...
let AddressIndexBenefactor;
if (__ELECTRON__) {
    AddressIndexBenefactor = require("worker-loader?inline!benefactors/AddressIndexBenefactor");
}
class AddressIndex extends BaseStore {
    ...
    */
    ...
    /** Benefactor thread implementation (for more than 10K keys) */
    addAll(pubkeys) {
        return new Promise((resolve, reject) => {
            ...
            this.loadAddyMap()
                .then(() => {
                    if (!__ELECTRON__) {
                        AddressIndexBenefactor = require("worker-loader!benefactors/AddressIndexBenefactor");
                    }
                    let benefactor = new AddressIndexBenefactor();
                    benefactor.postMessage({
                        pubkeys,
                        address_prefix: ChainConfig.address_prefix
                    });
                    // let _this = this
                    benefactor.onmessage = event => {
                        try {
